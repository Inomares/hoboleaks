...
function updateParentsFilter(clear) {
    ...
    for (let node of parents) {
        let childNode = node.getElementsByClassName("displayFilter")[0] as HTMLSpanElement
        if (clear) {
            if (childNode) {
...
function getMainSearchInput(): HTMLInputElement {
    return document.getElementById("mainSearchInput") as HTMLInputElement;
}

function filterFromHash(searchTerm: string) {
...
function userSearch(searchTerm: string) {
    if (searchTerm) {
